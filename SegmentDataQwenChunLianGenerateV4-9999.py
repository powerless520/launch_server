...
class ChunlianGenerator:
    def __init__(self):
        ...
        self.image_width_h = 160
        self.image_height_h = 160
    ...
    def create_duilian(self, text_images,save_name):
        ...
        image_height = self.image_height * (len(text_images)-1)
        image_width = self.image_width

        # spring_festival_image = Image.new('RGB', (image_width+40, image_height+30), color=(114, 20, 0))
        spring_festival_image = Image.new('RGBA', (image_width, image_height), color=(0, 0, 0,0))

        ImageDraw.Draw(spring_festival_image)
    ...
    def create_hengpi(self, text_images,save_name):
        ...
        image_height = self.image_height * 7
        image_width = self.image_width
    ...
    def create_hengpi1(self, text_images,save_name):
        ims=[]
        for im in text_images:
            ims.append(self.padding(im))
        text_images = ims
        ...
        # 春联图片保存到文件
        # spring_festival_image = spring_festival_image.resize((image_width//2, image_height//2))
        spring_festival_image.save(save_path)
        self.save2class(save_name,spring_festival_image)
    ...
    def merg_chunlian(self):
        ...
        height = self.shanglian_image.height+100
        width = horizontal_image.width * 4
    ...
    def merg_chunlian1(self):
        ...
        # 创建一个iPhone屏幕大小的画布
        # canvas = Image.new("RGB", (1400, 1300), "white")
        height = horizontal_image.height+ self.shanglian_image.height+100
        canvas = Image.new("RGBA", (horizontal_image.width, height), (0, 0, 0, 0))

   
        # 计算上联图像的位置
        x1 = int(horizontal_image.width/2-horizontal_image.width/2)
        y1 = 160
        x2 = x1 + image1.width
        y2 = y1 + image1.height

        # 计算下联图像的位置
        x3 = horizontal_image.width -image2.width-int(horizontal_image.width/2-horizontal_image.width/2)
        y3 = 160
        x4 = x3 + image2.width
        y4 = y3 + image2.height

        # 计算横批图像的位置
        x5 = int(horizontal_image.width/2-horizontal_image.width/2)
        y5 = 20
        x6 = x5 + horizontal_image.width
        y6 = y5 + horizontal_image.height
    ...
    def merg_result(self):
        save_path = os.path.join(RESULT_PATH,self.formatted_time+'-'+self.custom_text+'-输出'+'-'+self.hengpi+'.png')
        ...
        chunlian = chunlian.resize((new_width//2, new_height//2))
        chunlian.save('test.png')
        # canvas = Image.new("RGBA", (1284+300, height), (0, 0, 0, 0))
        canvas = Image.open('9999.png')

        print("Canvas mode:", canvas.mode)
        print("Canvas size:", canvas.size)
        print("Chunlian mode:", chunlian.mode)
        print("Chunlian size:", chunlian.size)
                
         # 计算上联图像的位置
        x1 = 500
        y1 = 20

        canvas.paste(chunlian, (x1, y1),mask=chunlian)
        font1 = ImageFont.truetype("font/京華老宋体v1.007.ttf", size=20)
        font2 = ImageFont.truetype("font/京華老宋体v1.007.ttf", size=30)
        # 创建绘图对象
        draw = ImageDraw.Draw(canvas)
        # 写入文字
        text = '“太白虽云鹤蓬莱,其威望名满天下,代代相传。”'
        position = (20, 700)  # 文字的左上角坐标
        text_color = (47, 31, 9)  # 文字颜色，RGB格式
        draw.text(position, text, fill=text_color, font=font1)
        draw.text((20,740), "注：书法笔迹由人工智能通过小样本学习当今李白真迹《上阳台帖》所生成。", fill=text_color, font=font1)

        draw = self.heng2shu(draw,(250,40),font2,text_color)
        canvas.save(save_path)
        return save_path
    ...
    def heng2shu(self, draw, position, font, text_color):
        ...
        for text in texts:
            for char in text:
                ...
                y_text += 35 # 假设每个字符的高度约为20像素
            x_text += 50  # 更新x坐标，移动到下一列
...
对联格式要求：上联7个字，下联7个字，横批四个字。除非用户单独提要求，否则请务必按照此格式书写对联。输出时请按下面这种格式输出
{
    "上联":"你写的上联"
    "下联":"你写的下联"
    "横批":"你写的横批"
...
必须要做到上下联的字数限制，上联7个字，下联7个字，横批四个字，把用户的名字藏在对联之中，注意不要连续出现，每个字只出现一次，并且要分开，不能出现在横批上,不要标点符号，同时保证平仄、对仗，文采
...
def pipeline(custom_text):
    # try:
        ...
        return True, generator.merg_result()
...
def rm_red(img):
    ...
    for x in range(img.width):
        for y in range(img.height):
            ...
            # 红色像素通常红色分量远大于绿色和蓝色分量
            if r > 90 and g < 50 and b < 50:
                # 红色像素变透明
                continue
            else:
                # 保留其他颜色的像素
...
if __name__ == "__main__":
    ...
    # 加载选中的图片
    shanglian_image = [rm_red(Image.open(image_path)) for image_path in shanglian]
    xialian_image = [rm_red(Image.open(image_path)) for image_path in xialian]
    hengpi_image = [rm_red(Image.open(image_path)) for image_path in hengpi]
