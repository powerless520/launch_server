...
class ChunlianGenerator:
    ...
    def merg_result(self):
        ...
        x1 = 400
        y1 = 0
        x2 = x1 + self.chunlian.width
        y2 =self.chunlian.height
        canvas.paste(self.chunlian, (x1, y1, x2, y2))
        font = ImageFont.truetype("font/Arial_Unicode.ttf", size=100)
        # 创建绘图对象
        draw = ImageDraw.Draw(canvas)
        # 写入文字
        text = '太白书之'
        position = (150, 300)  # 文字的左上角坐标
        text_color = (255, 255, 255)  # 文字颜色，RGB格式
        draw.text(position, text, fill=text_color, font=font)
        draw = self.heng2shu(draw,(180,600),font,text_color)
        canvas.save(save_path)
        return save_path
